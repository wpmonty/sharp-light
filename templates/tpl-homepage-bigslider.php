...
<?php get_header(); ?>
		...
		<div class="col-md-12">
			<div class="big-featured-slider">			
				...
				if (get_option('sharp_dnd') == 1) { $do_not_duplicate[] = $post->ID; }
					...
					get_template_part( 'loops/featured-slider-bigslider' );
	...
	(function($) {
		$(document).ready(function() {
			$('.featured-slider, .big-featured-slider').owlCarousel({
				loop				: true,
				nav					: true,
				navText: [
					...
					<div class="widget text-center">
						<?php if ( get_option('sharp_cap3img') <> "" ) { ?>
							<a href="<?php echo get_category_link(get_option('sharp_cat3'));?>">
								<img src="<?php echo get_option('sharp_cap3img'); ?>" class="aligncenter" alt="" />
							</a>
						<?php } else { ?>
							<p class="catname">
								<a href="<?php echo get_category_link(get_option('sharp_cat3'));?>">
